...
def _parquet_engine() -> str:
    """Return the first usable parquet engine (pyarrow preferred, fastparquet fallback)."""
    try:
    ...
    try:
        ...
        pass
    raise CacheError(
        "No parquet engine available. Install pyarrow or fastparquet: pip install pyarrow"
    )


_PARQUET_ENGINE = _parquet_engine()
...
class DiskCache:
    ...
    def get(self, key: str) -> pd.DataFrame | None:
        ...
        if raw is None:
            return None
        try:
            df = pd.read_parquet(io.BytesIO(raw), engine=_PARQUET_ENGINE)
            # fastparquet names the default RangeIndex "index" on round-trip.
    ...
    def set(self, key: str, df: pd.DataFrame, ttl: int | None = None) -> None:
        ...
        try:
            buf = io.BytesIO()
            df.to_parquet(buf, index=True, engine=_PARQUET_ENGINE)
            raw = buf.getvalue()
        except Exception as exc:
