...
echo.
echo  Stocks Dashboard - starting web interface...
echo  Browser will open at http://localhost:8501
echo  Keep this window open. Press Ctrl+C to stop.
echo.
"%PYTHON%" -m pip install -r requirements.txt -q
start "" "http://localhost:8501"
"%PYTHON%" -m streamlit run app.py --server.headless true --server.port 8501 --server.address 0.0.0.0 --browser.gatherUsageStats false
