...
def _inject_form_template_styles() -> None:
    ...
    st.markdown(
        ...
        .form-settings-row [data-testid="stTextInput"] input,
        .form-settings-row [data-testid="stNumberInput"] input {
            min-height: 2.1rem !important;
            font-size: 0.84rem !important;
        }
        .form-settings-row button {
            min-height: 2.1rem !important;
        ...
        /* --- Watchlist column (single-column compact panel) --- */
        ...
        /* Stacked form: keep watchlist as a narrow single column */
        [data-testid="stVerticalBlock"]:has(> div .stacked-watchlist-marker)
            [data-testid="column"]:has(.compact-watchlist-marker) {
            flex: 0 0 8.5rem !important;
            width: 8.5rem !important;
            min-width: 8.5rem !important;
...
class FormTemplate:
    ...
    ...     get_stocks=lambda: st.session_state["ten_min_watchlist"],
    ...     set_stocks=save_fn,
    ...     display_columns=["Symbol", "Current Price"],
    ...     column_config={...},
    ... ).render()
    """
    ...
    # Optional controls row (settings + refresh) for stacked/mobile layout.
    render_controls: ControlsFn | None = field(default=None, repr=False)

    display_columns: Sequence[str] | None = None
    column_config: ColumnConfig | None = None
    grid_use_container_width: bool = True
    row_px: int = 32
    header_px: int = 34
    max_table_height_px: int = 560
    report_ratio: float = 7.0
    watch_ratio: float = 0.9
    # "side" = report | watchlist columns; "stacked" = mobile: controls, grid, watchlist below.
    layout: str = "side"
    ...
    def bump_refresh(self) -> None:
        """Increment refresh token and invoke on_refresh callback."""
    ...
    def active_symbols(self) -> list[str]:
        """Included watchlist symbols, preferring live checkbox session state."""
        ...
        for stock in stocks:
            ...
            if key in st.session_state:
                if st.session_state[key]:
                    active.append(sym)
            elif stock.get("included", True):
                active.append(sym)
        return active

    def render_watchlist(self) -> list[str]:
        """Render compact single-column watchlist; return included symbols."""
        stocks: list[dict] = [dict(s) for s in self.get_stocks()]
    ...
    def render_grid(self, df: pd.DataFrame) -> None:
        ...
        table_height = min(table_height, self.max_table_height_px)
        kwargs: dict = {
            "use_container_width": self.grid_use_container_width,
            "hide_index": True,
            "height": table_height,
        }
    ...
    def _render_report(self, active: list[str]) -> None:
        """Load and display the report grid for the given symbols."""
        token_key = self._refresh_token_key()
        if not active:
